...
function TrendBadge({ score }: { score: number }) {
  if (score >= 50) return <span className="text-xs px-2 py-0.5 rounded-full bg-orange-500/20 text-orange-300 font-medium">🔥 Hot</span>
  if (score >= 20) return <span className="text-xs px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-300 font-medium">↑ Rising</span>
  if (score >= 5)  return <span className="text-xs px-2 py-0.5 rounded-full bg-gray-500/20 text-gray-400 font-medium">~ Stable</span>
            ...
            <div key={g.repo} className="bg-[#111118] rounded-xl border border-white/10 p-4">
              <div className="flex items-center justify-between gap-2 mb-3">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-sm font-medium text-white truncate">{g.repo}</span>
                </div>
              ...
              <div className="mb-3">
                ...
                <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${barPct}%`,
                      background: score >= 50 ? "#f97316" : score >= 20 ? "#7C3AED" : "#374151",
                    }}
                  />
                </div>
              </div>
